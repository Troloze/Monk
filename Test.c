...
int main() {
    ...
    SDL_Window * mWindow = SDL_CreateWindow("Troloze's Amazing Adventure(TM)", SDL_WINDOWPOS_UNDEFINED, SDL_WINDOWPOS_UNDEFINED, 1000, 1000, SDL_WINDOW_SHOWN);
    if(mWindow == NULL) {
        printf("Janela não pôde ser criada! Erro: %s\n", SDL_GetError());
        return false;
    }

    SDL_Renderer * mRenderer = SDL_CreateRenderer(mWindow, -1, SDL_RENDERER_SOFTWARE);
    if(mRenderer = NULL) {
    ...
    SDL_Surface * nSurface = IMG_Load("Test.png");
    if (nSurface == NULL) {
        printf("error %s\n", IMG_GetError());
    }
